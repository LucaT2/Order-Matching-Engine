// Deliberately mirrors apps/benchmark.cpp from optimized-array-implementation as closely as
// this branch's interface allows, so results are comparable rather than measuring two different
// workloads. Only real difference: this branch's OrderBook(cap) takes no price range, since
// std::map handles arbitrary prices natively -- the RNG still draws prices from the same
// [0, 29999] range so the workload itself is identical.
...
constexpr uint64_t MAX_PRICE = 29999;      // 30,000 price levels (100-tick-style range)
